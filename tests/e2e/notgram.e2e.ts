...
test("sender avatar stays attached across virtual blocks and hands off to the next group", async ({ page }) => {
  await page.setViewportSize({ width: 525, height: 620 });
  await page.goto("/");
  await page.evaluate(async (modulePath) => {
    const storeModule = await import(modulePath) as {
      telegramStore: {
    ...
    const state = storeModule.telegramStore.getState();
    const messages = new Map(state.messages);
    messages.set("chat-product", Array.from({ length: 80 }, (_, index) => ({
      id: `sticky-avatar-${index + 1}`,
      chatId: "chat-product",
      senderId: index < 40 ? "u-mia" : "u-chen",
      outgoing: false,
      sentAt: new Date(Date.UTC(2026, 7, 4, 0, 0, index)).toISOString(),
      delivery: "read",
      content: { kind: "text", text: `头像吸附消息 ${index + 1}` },
    })));
    const histories = new Map(state.histories);
  ...
  const floatingAvatar = page.locator(".message-sender-avatar-floating");
  const revealStickyAvatarMessage = async (messageId: string) => {
    const messageList = page.locator(".message-list");
    await expect(messageList).toHaveAttribute("aria-busy", "false");
    await messageList.evaluate((element) => {
      element.dispatchEvent(new WheelEvent("wheel", {
    ...
    await expect.poll(() => messageList.evaluate((element, targetId) => {
      ...
      // Text-only mock rows are close to 42px; the list overscan makes this
      // estimate deterministic while the target virtual item is mounted.
      element.scrollTop = Math.max(0, targetIndex * 42 - element.clientHeight / 2);
      element.dispatchEvent(new Event("scroll", { bubbles: true }));
      return false;
    }, messageId)).toBe(true);
    await page.evaluate(() => new Promise<void>((resolve) => {
  ...
  for (const messageId of ["sticky-avatar-4", "sticky-avatar-14", "sticky-avatar-24"]) {
    await revealStickyAvatarMessage(messageId);
    await expect(floatingAvatar).toHaveAccessibleName("查看 Mia Chen 资料");
    const geometry = await floatingAvatar.evaluate((avatar) => {
      const list = document.querySelector<HTMLElement>(".message-list")!;
      const avatarBounds = avatar.getBoundingClientRect();
      const listBounds = list.getBoundingClientRect();
      return {
        bottomGap: listBounds.bottom - avatarBounds.bottom,
        leftGap: avatarBounds.left - listBounds.left,
      };
    });
    expect(geometry.bottomGap).toBeCloseTo(4, 0);
    expect(geometry.leftGap).toBeCloseTo(10, 0);
  }

  await revealStickyAvatarMessage("sticky-avatar-54");
  await expect(floatingAvatar).toHaveAccessibleName("查看 陈默 资料");

  const messageList = page.locator(".message-list");
  await messageList.focus();
  await page.keyboard.press("End");
  await expect(floatingAvatar).toHaveCount(0);
  await expect(page.locator(
    '.message-group:has([data-message-id="sticky-avatar-80"]) .message-group-avatar .avatar',
  )).toContainText("陈");
});
